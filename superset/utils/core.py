...
def pessimistic_connection_handling(some_engine: Engine) -> None:
    """
    Install engine-level event listeners for the metadata database.

    Liveness checking of pooled connections is provided by SQLAlchemy's
    ``pool_pre_ping`` engine option, applied in
    ``SupersetAppInitializer.setup_db`` and overridable through
    ``SQLALCHEMY_ENGINE_OPTIONS``, rather than by an ``engine_connect``
    listener issuing its own ``SELECT 1``.

    :param some_engine: The metadata database engine
    """
